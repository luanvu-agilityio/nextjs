...
const showToast = (props: Omit<ToastProps, 'id'>) => {
  ...
  return toast.custom(
    t => (
      <div
        className={cn(
          'flex max-w-[360px] w-full gap-3 rounded-lg py-3 px-4 shadow-lg border',
          {
            'bg-success text-white border-success': variant === 'success',
            'bg-destructive-background text-white border-destructive-background':
              variant === 'error',
            'bg-warning text-white border-warning': variant === 'warning',
            'bg-info text-white border-info': variant === 'info',
          }
        )}
      >
